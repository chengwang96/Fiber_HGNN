...
class PointcloudRotate(object):
    def __call__(self, pc):
        rotation_angle_int = np.random.randint(-3, 4)
        rotation_angle = rotation_angle_int * np.pi / 180
        ...
        return pc, rotation_angle_int

class PointcloudScaleAndTranslate(object):
    def __init__(self, scale_low=9. / 10., scale_high=11. / 10., translate_range=0.1):
...
class PointcloudScale(object):
    def __init__(self, scale_low=9. / 10., scale_high=11. / 10.):
        self.scale_low = scale_low
        self.scale_high = scale_high

    def __call__(self, pc):
        xyz1 = np.random.uniform(low=self.scale_low, high=self.scale_high, size=[3])
        pc = torch.mul(pc, torch.from_numpy(xyz1).float().cuda())
...
class PointcloudTranslate(object):
    ...
    def __call__(self, pc):
        ...
        pc = pc + torch.from_numpy(xyz2).float().cuda()
